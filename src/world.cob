...
PROCEDURE DIVISION USING LK-CHUNK-X LK-CHUNK-Z LK-FAILURE.
            ...
            *> resetting the entire palette.
            IF SECTION-INDEX < WORLD-SECTION-COUNT
                IF PALETTE-LENGTH > 1
                    ...
                    END-PERFORM
                ELSE
                    MOVE 0 TO BLOCK-PALETTE-INDEX(WORLD-BLOCK-ID(CHUNK-INDEX, SECTION-INDEX, 1) + 1)
                END-IF
            END-IF
    ...
    *> Write the NBT data to disk
    ...
    CALL "Files-WriteAll" USING CHUNK-FILE-NAME NBT-BUFFER OFFSET
...
PROCEDURE DIVISION USING LK-CHUNK-X LK-CHUNK-Z LK-FAILURE.
    ...
    *> Read the file
    CALL "World-ChunkFileName" USING LK-CHUNK-X LK-CHUNK-Z CHUNK-FILE-NAME
    CALL "Files-ReadAll" USING CHUNK-FILE-NAME NBT-BUFFER NBT-BUFFER-LENGTH
...
PROCEDURE DIVISION USING LK-FAILURE.
    ...
    *> write the buffer to the file
    SUBTRACT 1 FROM OFFSET
    CALL "Files-WriteAll" USING LEVEL-FILE-NAME NBT-BUFFER OFFSET
...
PROCEDURE DIVISION USING LK-FAILURE.
    ...
    *> Read the file
    CALL "Files-ReadAll" USING LEVEL-FILE-NAME NBT-BUFFER NBT-BUFFER-LENGTH
    IF NBT-BUFFER-LENGTH = 0
        GOBACK
    END-IF
