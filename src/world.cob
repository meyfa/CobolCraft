...
PROCEDURE DIVISION USING LK-CHUNK-X LK-CHUNK-Z LK-CHUNK-INDEX.
    PERFORM VARYING LK-CHUNK-INDEX FROM 1 BY 1 UNTIL LK-CHUNK-INDEX > WORLD-CHUNK-COUNT
        SET ADDRESS OF WORLD-CHUNK TO WORLD-CHUNK-POINTER(LK-CHUNK-INDEX)
        IF LK-CHUNK-X = WORLD-CHUNK-X AND LK-CHUNK-Z = WORLD-CHUNK-Z
            GOBACK
        END-IF
    END-PERFORM
    ...
    *> If not present, allocate a new chunk
    IF LK-CHUNK-INDEX > WORLD-CHUNK-COUNT
        IF LK-CHUNK-INDEX > WORLD-CHUNK-CAPACITY
            ...
            GOBACK
        END-IF
        ALLOCATE WORLD-CHUNK
        SET WORLD-CHUNK-POINTER(LK-CHUNK-INDEX) TO ADDRESS OF WORLD-CHUNK
    END-IF

    INITIALIZE WORLD-CHUNK
    MOVE LK-CHUNK-X TO WORLD-CHUNK-X
    MOVE LK-CHUNK-Z TO WORLD-CHUNK-Z
...
PROCEDURE DIVISION USING LK-CHUNK-X LK-CHUNK-Z.
    ...
    *> turn all blocks with Y <= 63 (= the bottom 128 blocks = the bottom 8 sections) into stone
    ...
    PERFORM VARYING SECTION-INDEX FROM 1 BY 1 UNTIL SECTION-INDEX > 8
        ...
        END-PERFORM
        MOVE 4096 TO WORLD-SECTION-NON-AIR(SECTION-INDEX)
    END-PERFORM
...
PROCEDURE DIVISION USING LK-CHUNK-INDEX LK-FAILURE.
    ...
    *> chunk position
    ...
    CALL "NbtEncode-Int" USING NBT-ENCODER-STATE NBT-BUFFER TAG-NAME NAME-LEN WORLD-CHUNK-X
    MOVE "zPos" TO TAG-NAME
    CALL "NbtEncode-Int" USING NBT-ENCODER-STATE NBT-BUFFER TAG-NAME NAME-LEN WORLD-CHUNK-Z
    ...
    *> start chunk sections
    ...
    PERFORM VARYING SECTION-INDEX FROM 1 BY 1 UNTIL SECTION-INDEX > WORLD-SECTION-COUNT
        ...
        *> Note: The official format stores all sections, but it seems unnecessary, so we don't.
        IF WORLD-SECTION-NON-AIR(SECTION-INDEX) > 0
            *> start section
            ...
            *> palette
            ...
            PERFORM VARYING BLOCK-INDEX FROM 1 BY 1 UNTIL BLOCK-INDEX > 4096
                *> If the block is not in the palette, add it
                MOVE WORLD-BLOCK-ID(SECTION-INDEX, BLOCK-INDEX) TO CURRENT-BLOCK-ID
            ...
            *> palette
            ...
            PERFORM VARYING BIOME-INDEX FROM 1 BY 1 UNTIL BIOME-INDEX > 64
                *> If the biome is not in the palette, add it
                MOVE WORLD-BIOME-ID(SECTION-INDEX, BIOME-INDEX) TO CURRENT-BIOME-ID
    ...
    *> start block entities
    ...
    IF WORLD-BLOCK-ENTITY-COUNT > 0
        MOVE 1 TO BLOCK-INDEX
        MOVE 0 TO ENTITY-COUNT
        PERFORM 98304 TIMES
            IF WORLD-BLOCK-ENTITY-ID(BLOCK-INDEX) >= 0
                *> start block entity
                ...
                *> x and z are in the world coordinate system
                COMPUTE ENTITY-X = ENTITY-X + WORLD-CHUNK-X * 16
                COMPUTE ENTITY-Z = ENTITY-Z + WORLD-CHUNK-Z * 16
                ...
                *> stop the loop once all block entities have been written
                ...
                IF ENTITY-COUNT >= WORLD-BLOCK-ENTITY-COUNT
                    EXIT PERFORM
                END-IF
            END-IF
            ADD 1 TO BLOCK-INDEX
    ...
    *> Save the chunk
    ...
    CALL "Region-WriteChunkData" USING WORLD-CHUNK-X WORLD-CHUNK-Z NBT-BUFFER NBT-BUFFER-LENGTH LK-FAILURE
    IF LK-FAILURE NOT = 0
        GOBACK
    END-IF

    MOVE 0 TO WORLD-CHUNK-DIRTY
...
PROCEDURE DIVISION USING LK-CHUNK-X LK-CHUNK-Z LK-FAILURE.
    ...
    *> start sections
    ...
    PERFORM LOADED-SECTION-COUNT TIMES
        ...
        *> section block states
        MOVE NBT-DECODER-STATE TO NBT-SEEK-STATE
        CALL "DecodeSectionBlockStates" USING NBT-SEEK-STATE NBT-BUFFER WORLD-SECTION-BLOCKS(SECTION-INDEX)
                WORLD-SECTION-NON-AIR(SECTION-INDEX) LK-FAILURE
        IF LK-FAILURE > 0
            GOBACK
        END-IF

        *> section biomes
        CALL "DecodeSectionBiomes" USING NBT-DECODER-STATE NBT-BUFFER WORLD-SECTION-BIOMES(SECTION-INDEX) LK-FAILURE
        IF LK-FAILURE > 0
            GOBACK
        END-IF
    ...
    *> TODO: make this position-independent
    ...
    IF AT-END = 0
        ...
        MOVE ENTITY-COUNT TO WORLD-BLOCK-ENTITY-COUNT
        PERFORM ENTITY-COUNT TIMES
            ...
            *> convert to chunk-relative coordinates
            ...
            MOVE ENTITY-ID TO WORLD-BLOCK-ENTITY-ID(INT32)
            CALL "NbtDecode-EndCompound" USING NBT-DECODER-STATE NBT-BUFFER
        END-PERFORM
        CALL "NbtDecode-EndList" USING NBT-DECODER-STATE NBT-BUFFER
    END-IF
...
PROCEDURE DIVISION USING LK-SAVE-REQUIRED.
    ...
    PERFORM VARYING CHUNK-X FROM SPAWN-X-START BY 1 UNTIL CHUNK-X > SPAWN-X-END
        PERFORM VARYING CHUNK-Z FROM SPAWN-Z-START BY 1 UNTIL CHUNK-Z > SPAWN-Z-END
            CALL "World-EnsureChunk" USING CHUNK-X CHUNK-Z CHUNK-INDEX
            IF CHUNK-INDEX = 0
                GOBACK
            END-IF
            SET ADDRESS OF WORLD-CHUNK TO WORLD-CHUNK-POINTER(CHUNK-INDEX)
            IF WORLD-CHUNK-DIRTY > 0
...
PROCEDURE DIVISION USING LK-VIEW-DISTANCE LK-FAILURE.
    ...
    *> Do not unload spawn chunks
    ...
    PERFORM VARYING CHUNK-INDEX FROM 1 BY 1 UNTIL CHUNK-INDEX > WORLD-CHUNK-COUNT
        SET ADDRESS OF WORLD-CHUNK TO WORLD-CHUNK-POINTER(CHUNK-INDEX)
        IF (WORLD-CHUNK-X < SPAWN-X-START OR > SPAWN-X-END) OR (WORLD-CHUNK-Z < SPAWN-Z-START OR > SPAWN-Z-END)
            COMPUTE CHUNK-BLOCK-X = WORLD-CHUNK-X * 16 + 8
            COMPUTE CHUNK-BLOCK-Z = WORLD-CHUNK-Z * 16 + 8
...
PROCEDURE DIVISION USING LK-POSITION LK-BLOCK-ID.
    ...
    *> compute the block index
    ...
    MOVE WORLD-BLOCK-ID(SECTION-INDEX, BLOCK-INDEX) TO LK-BLOCK-ID
    GOBACK.
...
PROCEDURE DIVISION USING LK-CLIENT LK-POSITION LK-BLOCK-ID.
    ...
    *> Skip if identical to the current block
    MOVE WORLD-BLOCK-ID(SECTION-INDEX, BLOCK-INDEX) TO PREVIOUS-BLOCK-ID
    ...
    *> Check whether the block is becoming air or non-air
    EVALUATE TRUE
        WHEN LK-BLOCK-ID = 0
            SUBTRACT 1 FROM WORLD-SECTION-NON-AIR(SECTION-INDEX)
        WHEN PREVIOUS-BLOCK-ID = 0
            ADD 1 TO WORLD-SECTION-NON-AIR(SECTION-INDEX)
    END-EVALUATE

    *> Set the block and mark the chunk as dirty
    MOVE LK-BLOCK-ID TO WORLD-BLOCK-ID(SECTION-INDEX, BLOCK-INDEX)
    MOVE 1 TO WORLD-CHUNK-DIRTY
    ...
    *> If the block is changing to a different type (not just state), remove any block entity
    IF PREVIOUS-BLOCK-ID NOT = 0
        ...
        IF IS-SAME-BLOCK-TYPE = 0
            ...
            IF WORLD-BLOCK-ENTITY-ID(BLOCK-IN-CHUNK-INDEX) >= 0
                MOVE -1 TO WORLD-BLOCK-ENTITY-ID(BLOCK-IN-CHUNK-INDEX)
                SUBTRACT 1 FROM WORLD-BLOCK-ENTITY-COUNT
            END-IF
        END-IF
    END-IF
...
PROCEDURE DIVISION USING LK-POSITION LK-BLOCK-ENTITY-ID.
    ...
    *> Set the block entity ID
    MOVE LK-BLOCK-ENTITY-ID TO WORLD-BLOCK-ENTITY-ID(BLOCK-IN-CHUNK-INDEX)
    ADD 1 TO WORLD-BLOCK-ENTITY-COUNT
