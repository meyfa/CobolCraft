        ...
        *> Skip spaces
        IF LK-HEX-STRING-IN(INPUT-INDEX:1) NOT = " "
            *> Convert the current character to the upper part of the byte
            CALL STATIC "DecodeHexChar" USING LK-HEX-STRING-IN(INPUT-INDEX:1) NIBBLE-VALUE-HIGH
            *> Convert the next character to the lower part of the byte
            ADD 1 TO INPUT-INDEX
            CALL STATIC "DecodeHexChar" USING LK-HEX-STRING-IN(INPUT-INDEX:1) NIBBLE-VALUE-LOW
...
PROCEDURE DIVISION USING LK-BUFFER-IN LK-BUFFER-LEN LK-HEX-STRING-OUT.
    PERFORM VARYING BYTE-INDEX FROM 1 BY 1 UNTIL BYTE-INDEX > LK-BUFFER-LEN
        ...
        CALL STATIC "EncodeHexChar" USING NIBBLE-VALUE LK-HEX-STRING-OUT(STRING-INDEX:1)
        ADD 1 TO STRING-INDEX
        CALL STATIC "EncodeHexChar" USING BYTE-VALUE LK-HEX-STRING-OUT(STRING-INDEX:1)
        ADD 1 TO STRING-INDEX
    END-PERFORM
    GOBACK.
