...
LINKAGE SECTION.
    ...
    01 LK-IO-FAILURE            BINARY-CHAR UNSIGNED.

PROCEDURE DIVISION USING LK-UUID LK-NAME LK-ALREADY-EXISTS LK-IO-FAILURE.
    CALL "Whitelist-Check" USING LK-UUID LK-NAME LK-ALREADY-EXISTS
    IF LK-ALREADY-EXISTS NOT = 0
        GOBACK
    END-IF
    ADD 1 TO WHITELIST-LENGTH
    MOVE LK-UUID TO WHITELIST-UUID(WHITELIST-LENGTH)
    MOVE LK-NAME TO WHITELIST-NAME(WHITELIST-LENGTH)
    CALL "Whitelist-Write" USING LK-IO-FAILURE
    GOBACK.
...
PROCEDURE DIVISION USING LK-UUID LK-NAME LK-NOT-FOUND LK-IO-FAILURE.
    PERFORM VARYING WHITELIST-INDEX FROM 1 BY 1 UNTIL WHITELIST-INDEX > WHITELIST-LENGTH
        IF WHITELIST-NAME(WHITELIST-INDEX) = LK-NAME
