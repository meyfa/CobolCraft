...
PROCEDURE DIVISION USING LK-JSON LK-JSON-LEN LK-FAILURE.
    ...
    MOVE 0 TO BLOCKS-MAXIMUM-STATE-ID
...
*> --- Blocks-Get-MaximumStateId ---
IDENTIFICATION DIVISION.
PROGRAM-ID. Blocks-Get-MaximumStateId.
...
END PROGRAM Blocks-Get-MaximumStateId.
...
*> --- Blocks-Get-StateIds ---
IDENTIFICATION DIVISION.
PROGRAM-ID. Blocks-Get-StateIds.
...
PROCEDURE DIVISION USING LK-BLOCK-NAME LK-MINIMUM-ID LK-MAXIMUM-ID.
    MOVE -1 TO LK-MINIMUM-ID LK-MAXIMUM-ID
    PERFORM VARYING BLOCK-INDEX FROM 1 BY 1 UNTIL BLOCK-INDEX > BLOCKS-COUNT
        IF LK-BLOCK-NAME = BLOCK-ENTRY-NAME(BLOCK-INDEX)
            MOVE BLOCK-ENTRY-MINIMUM-STATE-ID(BLOCK-INDEX) TO LK-MINIMUM-ID
            MOVE BLOCK-ENTRY-MAXIMUM-STATE-ID(BLOCK-INDEX) TO LK-MAXIMUM-ID
            EXIT PERFORM
        END-IF
    END-PERFORM
    GOBACK.

END PROGRAM Blocks-Get-StateIds.
...
*> --- Blocks-StateIdToIndex ---
*> Get the index into the blocks array for a given state ID (i.e., find the parent block type).
IDENTIFICATION DIVISION.
PROGRAM-ID. Blocks-StateIdToIndex.
...
PROCEDURE DIVISION USING LK-STATE-ID LK-INDEX.
    PERFORM VARYING LK-INDEX FROM 1 BY 1 UNTIL LK-INDEX > BLOCKS-COUNT
        IF LK-STATE-ID >= BLOCK-ENTRY-MINIMUM-STATE-ID(LK-INDEX) AND LK-STATE-ID <= BLOCK-ENTRY-MAXIMUM-STATE-ID(LK-INDEX)
            GOBACK
        END-IF
    END-PERFORM
    MOVE 0 TO LK-INDEX
...
END PROGRAM Blocks-StateIdToIndex.
...
PROCEDURE DIVISION USING LK-BLOCK-INDEX LK-HARDNESS.
    MOVE LK-HARDNESS TO BLOCK-ENTRY-HARDNESS(LK-BLOCK-INDEX)
    GOBACK.
...
PROCEDURE DIVISION USING LK-BLOCK-INDEX LK-HARDNESS.
    MOVE BLOCK-ENTRY-HARDNESS(LK-BLOCK-INDEX) TO LK-HARDNESS
    GOBACK.
