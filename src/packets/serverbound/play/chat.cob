IDENTIFICATION DIVISION.
PROGRAM-ID. RecvPacket-Chat.

DATA DIVISION.
WORKING-STORAGE SECTION.
    COPY DD-CLIENTS.
    COPY DD-PLAYERS.
    COPY DD-SERVER-PROPERTIES.
    COPY DD-CLIENT-STATES.
    01 PLAYER-ID                BINARY-LONG.
    *> payload
    01 MESSAGE-BUFFER           PIC X(256).
    01 MESSAGE-LENGTH           BINARY-LONG UNSIGNED.
    01 TIMESTAMP                BINARY-LONG-LONG.
    01 SALT                     BINARY-LONG-LONG.
    01 SIGNATURE-PRESENT        BINARY-CHAR UNSIGNED.
    *> 01 SIGNATURE                PIC X(256).
    01 MESSAGE-COUNT            BINARY-LONG.
    *> 01 ACKNOWLEDGED             PIC X(3).
    *> variables
    01 CLIENT-ID                BINARY-LONG UNSIGNED.
LINKAGE SECTION.
    01 LK-CLIENT                BINARY-LONG UNSIGNED.
    01 LK-BUFFER                PIC X ANY LENGTH.
    01 LK-OFFSET                BINARY-LONG UNSIGNED.

PROCEDURE DIVISION USING LK-CLIENT LK-BUFFER LK-OFFSET.
    MOVE CLIENT-PLAYER(LK-CLIENT) TO PLAYER-ID

    CALL "Decode-String" USING LK-BUFFER LK-OFFSET MESSAGE-LENGTH MESSAGE-BUFFER
    CALL "Decode-Long" USING LK-BUFFER LK-OFFSET TIMESTAMP
    CALL "Decode-Long" USING LK-BUFFER LK-OFFSET SALT
    CALL "Decode-Byte" USING LK-BUFFER LK-OFFSET SIGNATURE-PRESENT
    IF SIGNATURE-PRESENT NOT = 0
        ADD 256 TO LK-OFFSET
    END-IF
    CALL "Decode-VarInt" USING LK-BUFFER LK-OFFSET MESSAGE-COUNT
    *> acknowledged
    ADD 3 TO LK-OFFSET
    *> checksum
    ADD 1 TO LK-OFFSET

    IF MESSAGE-LENGTH > 256
        CALL "Server-DisconnectClient" USING LK-CLIENT
        GOBACK
    END-IF

    *> display the message in the server console
    DISPLAY "<" FUNCTION TRIM(PLAYER-NAME(PLAYER-ID)) "> " MESSAGE-BUFFER(1:MESSAGE-LENGTH)

    *> send the message to all clients in play state
    PERFORM VARYING CLIENT-ID FROM 1 BY 1 UNTIL CLIENT-ID > MAX-CLIENTS
        IF CLIENT-STATE(CLIENT-ID) = CLIENT-STATE-PLAY
            CALL "SendPacket-PlayerChat" USING CLIENT-ID CHAT-MESSAGES-SENT(CLIENT-ID) PLAYER-UUID(PLAYER-ID) PLAYER-NAME(PLAYER-ID) MESSAGE-BUFFER MESSAGE-LENGTH
            ADD 1 TO CHAT-MESSAGES-SENT(CLIENT-ID)
        END-IF
    END-PERFORM

    GOBACK.

END PROGRAM RecvPacket-Chat.
