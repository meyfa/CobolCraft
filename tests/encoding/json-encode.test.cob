...
PROCEDURE DIVISION.
    ...
    *> --- Test: JsonEncode-Integer ---
    ...
    Max.
        *> Largest 32-bit value: all 10 digits must survive.
        COPY TEST-CASE REPLACING ==NAME== BY =="2147483647"==.
        MOVE 2147483647 TO INT64
        INITIALIZE BUFFER
        MOVE 2 TO OFFSET
        CALL "JsonEncode-Integer" USING BUFFER OFFSET INT64
        COPY TEST-ASSERT REPLACING COND BY ==OFFSET = 12 AND BUFFER = " 2147483647"==.
    Min.
        *> Smallest 32-bit value: 10 digits plus sign.
        COPY TEST-CASE REPLACING ==NAME== BY =="-2147483648"==.
        MOVE -2147483648 TO INT64
        INITIALIZE BUFFER
        MOVE 2 TO OFFSET
        CALL "JsonEncode-Integer" USING BUFFER OFFSET INT64
        COPY TEST-ASSERT REPLACING COND BY ==OFFSET = 13 AND BUFFER = " -2147483648"==.
    TenDigitNegative.
        COPY TEST-CASE REPLACING ==NAME== BY =="-1000000000"==.
        MOVE -1000000000 TO INT64
        INITIALIZE BUFFER
        MOVE 2 TO OFFSET
        CALL "JsonEncode-Integer" USING BUFFER OFFSET INT64
        COPY TEST-ASSERT REPLACING COND BY ==OFFSET = 13 AND BUFFER = " -1000000000"==.
