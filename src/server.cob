    ...
    PERFORM VARYING OTHER-CLIENT FROM 1 BY 1 UNTIL OTHER-CLIENT > MAX-CLIENTS
        IF CLIENT-PRESENT(OTHER-CLIENT) = 1 AND CLIENT-STATE(OTHER-CLIENT) = CLIENT-STATE-PLAY
            ...
            IF OTHER-CLIENT NOT = LK-CLIENT
                CALL "SendPacket-SpawnEntity" USING LK-CLIENT OTHER-PLAYER-ID PLAYER-UUID(OTHER-PLAYER-ID)
                    ENTITY-TYPE-PLAYER PLAYER-POSITION(OTHER-PLAYER-ID) PLAYER-ROTATION(OTHER-PLAYER-ID)
            END-IF
        END-IF
    END-PERFORM
    ...
    PERFORM VARYING OTHER-CLIENT FROM 1 BY 1 UNTIL OTHER-CLIENT > MAX-CLIENTS
        IF CLIENT-PRESENT(OTHER-CLIENT) = 1 AND CLIENT-STATE(OTHER-CLIENT) = CLIENT-STATE-PLAY AND OTHER-CLIENT NOT = LK-CLIENT
            ...
            *> spawn a player entity
            CALL "SendPacket-SpawnEntity" USING OTHER-CLIENT PLAYER-ID PLAYER-UUID(PLAYER-ID)
                ENTITY-TYPE-PLAYER PLAYER-POSITION(PLAYER-ID) PLAYER-ROTATION(PLAYER-ID)
        END-IF
    END-PERFORM
