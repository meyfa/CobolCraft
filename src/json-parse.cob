...
PROCEDURE DIVISION USING LK-INPUT LK-LENGTH LK-OFFSET.
    PERFORM UNTIL LK-OFFSET > LK-LENGTH
        *> Exit the loop once a non-whitespace character is found
        IF NOT (LK-INPUT(LK-OFFSET:1) = X"20" OR LK-INPUT(LK-OFFSET:1) = X"09" OR LK-INPUT(LK-OFFSET:1) = X"0A" OR LK-INPUT(LK-OFFSET:1) = X"0D")
            EXIT PERFORM
        END-IF
        ADD 1 TO LK-OFFSET
    END-PERFORM
...
PROCEDURE DIVISION USING LK-INPUT LK-OFFSET LK-FLAG.
    ...
    CALL "JsonParse-SkipWhitespace" USING LK-INPUT INPUT-LENGTH LK-OFFSET
